...
function Chat(props) {
  ...
  function handleSelectUser(user, conversation) {
    setchatUser(user);
    ...
    >
      ...
      >
        ...
        <div className="flex flex-col h-full">
          ...
          <div className="grid md:grid-cols-[30%_45%_25%] h-full mt-7 gap-x-5">
            {/* Column 1*/}
            {isConversationList && (
            ...
            {!currentConversation && !chatUser && (
              <div className="border-2 rounded-xl border-pink-200 col-span-2 flex flex-col justify-center items-center bg-zinc-100 dark:bg-slate-900">
                ...
                <h1 className="text-4xl dark:text-gray-50 pt-10">
                  Messaging App
                </h1>
                <p className="mt-4 dark:text-zinc-400">
                  Send and receive messages and connect to each others
                </p>
              </div>
            )}
            {/* Column 2*/}
            {((isChatWindow && currentConversation) || chatUser) && (
              <div
                className={`${
              ...
              >
                <ChatWindow
                  chatUser={chatUser}
                  controls={{ handleBacktoConversation, handleOpenContactInfo }}
                ></ChatWindow>
              </div>
            )}
            {/* Column 3 */}
            {isOpenContactInfo && chatUser && (
              <ContactInfo
